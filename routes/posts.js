...
const { requireAuth } = require('./auth'); // Importation de ton middleware JWT
...
router.post('/', requireAuth, upload.single('video'), async (req, res) => {
    const userId = req.user.id; // MODIFIÉ : req.user au lieu de req.session.user
    const { caption, hashtags, scheduled_at, title } = req.body;

    if (!req.file) return res.status(400).json({ error: 'Vidéo obligatoire' });
    if (!scheduled_at) return res.status(400).json({ error: 'Date obligatoire' });

    const scheduledDate = new Date(scheduled_at);
    if (scheduledDate <= new Date()) return res.status(400).json({ error: 'La date doit être dans le futur' });

    try {
        const hashtagsArray = hashtags ? hashtags.split(',').map(h => h.trim().replace(/^#/, '')) : [];
...
router.get('/', requireAuth, async (req, res) => {
    ...
    try {
        let query = `SELECT id, title, caption, hashtags, scheduled_at, status, tiktok_post_id, error_message, created_at 
                     FROM scheduled_posts WHERE user_id = $1`;
        const params = [userId];
...
router.delete('/:id', requireAuth, async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            ...
            RETURNING video_path`, [postId, userId]);

        if (!result.rows.length) return res.status(404).json({ error: 'Post introuvable' });
