  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (tiktok_open_id, display_name, avatar_url, access_token, refresh_token, token_expires_at) 
       ...
       DO UPDATE SET display_name=EXCLUDED.display_name, avatar_url=EXCLUDED.avatar_url, access_token=EXCLUDED.access_token, refresh_token=EXCLUDED.refresh_token, token_expires_at=EXCLUDED.token_expires_at, updated_at=NOW() 
    ...
    res.cookie('auth_token', token, { 
        httpOnly: true, 
        secure: true, 
        sameSite: 'none', // Important pour Railway si le front est sur un autre domaine
        maxAge: 604800000 
    });
...
function requireAuth(req, res, next) {
  ...
  try {
    ...
    // On vérifie que le token contient bien l'ID de l'utilisateur
    if (!decoded || !decoded.id) {
      return res.status(401).json({ error: 'Token invalide ou incomplet' });
    }

    // On attache l'utilisateur à req.user pour les fichiers suivants
    req.user = decoded; 
    next();
  } catch (err) {
...
// Exportation multiple : le router ET la fonction de protection
